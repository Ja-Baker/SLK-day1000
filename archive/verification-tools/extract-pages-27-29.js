...
/**
 * Extract Exact Content for Pages 27-29
 * Pages 27 (Grammar/Syntax), 28 (Vocabulary), 29 (Literacy Foundations)
 */
...
class ContentExtractor {
    constructor() {
        ...
        this.outputPath = path.join(__dirname, '../extracted-content/pages-27-29-content.md');
    }

    async extractContent() {
        console.log('📖 Extracting exact content for Pages 27-29...');

        const markdownContent = fs.readFileSync(this.sourceMarkdown, 'utf8');
        const extractedContent = [];

        // Extract Page 27 (Grammar/Syntax)
        extractedContent.push('# Page 27 - Grammar/Syntax');
        extractedContent.push('');
        extractedContent.push(this.extractPageContent(markdownContent, 27));
        extractedContent.push('');
        extractedContent.push('---');
        extractedContent.push('');

        // Extract Page 28 (Vocabulary)
        extractedContent.push('# Page 28 - Vocabulary');
        extractedContent.push('');
        extractedContent.push(this.extractPageContent(markdownContent, 28));
        extractedContent.push('');
        extractedContent.push('---');
        extractedContent.push('');

        // Extract Page 29 (Literacy Foundations)
        extractedContent.push('# Page 29 - Literacy Foundations');
        extractedContent.push('');
        extractedContent.push(this.extractPageContent(markdownContent, 29));
